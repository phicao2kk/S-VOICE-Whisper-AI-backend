...
MODEL_SIZE = "base"  # Có thể đổi thành "tiny" để nhẹ hơn trên Render
...
def transcribe():
    try:
        ...
        result = model.transcribe(
            filepath,
            language='vi',  # Đổi thành 'vi' cho tiếng Việt, hoặc để None để auto
            task='transcribe',
            fp16=False,
            temperature=0.0
        )
...
def home():
    return jsonify({
        'message': '🎤 S-VOICE Whisper Server',
        'endpoints': {
            'health': '/health',
...
if __name__ == '__main__':
    ...
    app.run(host='0.0.0.0', port=port, debug=False)  # Tắt debug trên production
